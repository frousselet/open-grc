...
- **Recessed sidebar**: the main menu loses its raised-card treatment (white surface, drop shadow). It now sits at page-background level inside a shallow well: same colour as the canvas, a hairline border and a soft inner shadow on the edges that suggests the slight depression, in both themes (new `--sidebar-recess-*` tokens). Content cards now read as the elevated layer, and menu links lift to a surface background on hover. On small screens the menu keeps a raised drawer style (surface background, large shadow) since it slides over the page.
