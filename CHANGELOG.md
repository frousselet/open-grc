...
- **Recessed sidebar**: the main menu loses its raised-card treatment (white surface, drop shadow) and becomes a recessed zone carved into the page: a background slightly darker than the canvas, a hairline border and a soft inner shadow, in both themes (new `--sidebar-recess-*` tokens). Content cards now read as the elevated layer. On small screens the menu keeps a raised drawer style (surface background, large shadow) since it slides over the page.
