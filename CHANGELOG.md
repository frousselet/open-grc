...
- **Recessed sidebar**: the main menu loses its raised-card treatment (white surface, drop shadow). The menu now keeps the page-background colour and rests inside a pressed-in tray shaped purely with shadows (inner shadows on all four edges plus a bottom light edge, per-theme `--sidebar-recess-*` tokens). The tray carries a small internal padding so the menu breathes inside it, and since the tray is the menu container it follows the collapsed width. Content cards now read as the elevated layer, and menu links lift to a surface background on hover. On small screens the menu keeps a raised drawer style (surface background, large shadow) since it slides over the page.
